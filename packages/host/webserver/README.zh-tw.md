...
- **不提供 TLS、認證或來源策略**：綁定非回環地址會向對應網路公開伺服器；面向部署的加固措施（或在前方放置真正的反向代理）有意不納入面向開發環境的 v1。
