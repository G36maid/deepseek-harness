...
`ctx.fs` 提供方約定（[`@deepseek-ai/dsh-fs`](../fs)）的**本機檔案系統實作**。它使用宿主檔案系統支援十二個 `FileSystem` 原語；將其作為外掛程式載入會填充 `ctx.fs`。
...
- **`readText` / `streamText`**：只支援 UTF-8。`readText` 讀取整個文件；`streamText` 按區塊解碼，因此超大文件無需整體保存在記憶體中，消費端也可以自行限制保留量。兩者都會拒絕無效 UTF-8、包含 NUL 位元組的二進位樣本（`FS_NOT_TEXT`）以及非普通文件目標。`read` 工具（`@deepseek-ai/dsh-tool-fs`）擁有行視窗邏輯。
...
包根 SDK 介面包含預設/具名 `LocalFileSystem` 類和 `Config`。原始 I/O 位於 `src/fsio.ts`（不相依性 Cordis，單獨進行單元測試）；`src/index.ts` 是輕量服務接線。
...
- **`editText` 會把整個文件及編輯後的副本保存在記憶體中**：只有讀取路徑支援流式處理。
...
- **帶防護的建立要求支援硬連結**：拒絕硬連結發布的檔案系統或掛載點無法支援 `createIfAbsent`；提供方會使目標保持缺失狀態並報告 `FS_IO_ERROR`。
