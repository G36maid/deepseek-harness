...
無。協定消費層只在瀏覽器與主機之間搬運已經組合好的訊息；這裡沒有任何內容進入模型請求。
...
- **`/api` 橋把每個請求體整體緩衝在記憶體裡**：`maxRequestBodyBytes`（預設 160 MiB，按預設 100 MiB 圖片總量上限經 base64 膨脹加信封餘量得出）因此同時是單請求的駐留記憶體上界；要降低它而不縮小圖片限額，需要流式請求體路徑。
