...
本包為 `WorkflowEngine` 提供實作，每次執行使用一個 Node worker thread。worker 執行編排指令碼；子 agent（代理）留在宿主上，指令碼透過帶類型的宿主／worker 協定經由 `ctx.subagents` 訪問它們。

包根目錄預設匯出引擎外掛程式及其 `Config`；worker 協定、執行時期和工作階段模組均為實作私有。操作入口 `./worker` 仍是引擎的 spawn 目標。
...
未知選項、格式錯誤的參數、不支援的 schema、超出上限、提供方啟動失敗和基礎設施結果失敗都屬於致命工作流程錯誤。有意不注入 timer、檔案系統 API 或 Node 全域性變數，但上述信任注意事項仍然適用。
...
子 agent 結果從宿主跨越到 worker 之前，會先投影並製作快照。這是真正近似行程的序列化邊界；它有意不同於可信的同行程工作流程和 subagent 事件 payload，後者以不可變方式借用值。
...
- **不注入預設可用的定時器、檔案系統或網路，但逃逸程式碼仍可訪問 Node**：這些缺失的全域性變數屬於可移植性 API 設計，而非隔離措施。
