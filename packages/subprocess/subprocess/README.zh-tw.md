...
- stdio 按流採用 Node 風格：`'pipe'` 把原始流交給呼叫方做自己的協定分幀（LSP 的 JSON-RPC、ACP（Agent Client Protocol）的 ndjson），`'inherit'` 直通父行程描述符以承載診斷輸出，收集模式（collect）`{ maxBytes, spill? }` 則緩衝一段有界尾部，外加選填的完整流 spill 文件。收集模式的讀取器接受全流位元組偏移量且從不消費，因此獨立的讀取器不會搶走彼此的增量；偏移量滑出記憶體尾部視窗的讀取標記為 `lossy`，並在 spill 文件存在時指向它。收集到的輸出在結帳後仍可讀取。
