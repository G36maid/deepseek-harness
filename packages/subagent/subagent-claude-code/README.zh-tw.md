...
本機取消會在結果競態中勝出並對映為 `aborted`。`dispose()`（資源釋放）具有冪等性：它會中止此次執行、請求 SDK query 關閉、呼叫共享的行程樹逐級終止機制，並等待整棵行程樹退出。SDK 的優雅關閉只表達協定意圖；行程是否完全靜止仍以子行程控制代碼為準。結果失敗與獨立的清理失敗仍彼此分離。
