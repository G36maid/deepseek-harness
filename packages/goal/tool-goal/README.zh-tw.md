...
- **不負責調度或直接面向人類呈現**：這些工具只變更狀態；同工作階段驅動器與 [`dsh-command-goal`](../command-goal/README.md) 是同一領域的獨立消費端。
- **Goal Round 權限需要驅動器**：除非續行驅動器准入 goal 來源的使用者輪次，否則自主 `complete`／`blocked` 路徑不會啟用；只掛載這個包不會建立這些輪次。
