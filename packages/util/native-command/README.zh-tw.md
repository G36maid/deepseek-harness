...
它的兩個消費端都是宿主側原生整合：[`directory-picker-native`](../../host/directory-picker-native/README.md) 後端的 OS 選擇器命令，以及閘道將路徑交由預設應用打開的操作（[`dsh-host-apiproxy`](../../host/apiproxy/README.md) 的 `host.openPath`）。`NativeCommandRunner` 類型是這些呼叫方的可注入命令邊界。
