...
- 每個 vitest 設定都將 vite-tsconfig-paths 指向 `tsconfig.base.json`；工作區包的裸匯入解析到 `src`（[版面設定](development.md#typescript-project-layout)），絕不會經由包的 `exports` 解析到建置後的 `lib/`，因為其中的過時產物會載入第二份模組單例。建置產物只在顯式指定時使用：以 `lib` 模式執行的子行程，以及下文的建置產物冒煙測試。
