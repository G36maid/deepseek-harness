...
[English](capability-seams.md) | 繁體中文
...
| `ctx.typert` | `core` | [`typert-registry`](../packages/typert/registry) | - | [`typert-loader`](../packages/typert/loader), [`api-gateway`](../packages/api/gateway) | - | 外掛程式直接或透過 dsh-typert-loader 註冊即時 zod 貢獻；API 閘道消費呼叫描述符和提供方，其他執行時期消費端則在各自邊界查詢 schema 與反射元資料。 |
...
| `ctx.planMode` | `core` | [`plan-mode`](../packages/plan/plan-mode) | - | - | - | 摺疊已記錄的計畫／模式狀態，在輪次邊界刷新使用者選擇，渲染由部署方擁有的指導資訊，註冊 /plan，並在狀態轉換期間保持計畫退出 schema 穩定。 |
...
| `ctx.sessionProjections` | `core` | [`session-projection`](../packages/session/session-projection) | - | [`tool-todo`](../packages/todo/tool-todo), [`session-title`](../packages/session/session-title), [`host-apiproxy`](../packages/host/apiproxy) | - | 各領域註冊由狀態驅動的摺疊單元；主動驅動過程維護每個工作階段的水位狀態，api-proxy 提供基線並推送發生變化的值。 |
...
| `ctx.skills` | `seam` | [`skill`](../packages/skill/skill) | [`skill-badge`](../packages/skill/skill-badge), [`skill-filesystem`](../packages/skill/skill-filesystem) | [`tool-skill`](../packages/skill/tool-skill) | - | 合併提供方的 skill（技能）目錄；tool-skill 渲染工作階段前綴目錄，並載入完整的 skill 正文。 |
