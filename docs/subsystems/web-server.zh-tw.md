...
`host` 只接受 `127.0.0.1`（預設姿態）和 `0.0.0.0`（刻意的網路暴露）；沒有 TLS、認證或 origin 策略，因此綁定到非回環地址會把伺服器暴露給該網路。dist 位置是認領席位的前端外掛程式的組裝事實。
