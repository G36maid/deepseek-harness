...
| 支持 | 支援 | support (verb) | 台湾惯用「支援」；「支持」台湾指「支持某观点」（动词 agree/back） |
| 协议 | 協定 | protocol | 台湾惯用「協定」；「協議」台湾指双方协商达成的约定 |
| 演示 | 示範 | demo/demonstration | 台湾惯用「示範」或「Demo」 |
| 进程 | 行程 | process | 台湾惯用「行程」；「進程」为大陆词（OS process） |
| 默认 | 預設 | default | 台湾惯用「預設」；「默認」虽为上游 zh-tw 既有用词，但台湾工程文档以「預設」为常态 |
