...
- 圍欄程式碼區塊：**逐位元組一致，包括註釋**。配對簽名比對資訊字串與內容，` ```ts ` 塊還要透過 `doc-typecheck` 編譯；
- 行內程式碼（命令、flag、設定鍵、檔案路徑、事件名、API 名、版本號）：原樣保留，從不翻譯或重排；
- 連結與錨點：每個相對連結在所有文件中必須指向相同的目標（按約定是 `.md` 路徑而非翻譯側文件），這樣即使某對文件先於相鄰文件落地，連結也不會懸空。只有語言切換列使用翻譯側鏈接。在 GitHub 以外位置渲染的 README 可以按 [README.md](README.md) 的規定，使用指向確切對側檔案的規範公開倉庫 URL。連結**文字**翻譯；連結目標不翻。
...
- 強調標記（`**加粗**`、`*斜体*`）落在與對側相同的文欄位上。中文沒有斜體，渲染效果可能看不出差別，不要用引號或其他裝飾替代。
