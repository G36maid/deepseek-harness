...
- 將倉庫根目錄設為 `cwd`，並設定 `launch_args_override=("./node_modules/.bin/tsx", "packages/examples/jsonrpc-demo/src/bin.ts")`，以執行未建置的 TypeScript 原始碼。預設設定不合適時，請提供 `cordis=...`。
...
為Pull Request新增 `python-release-dry-run` 標籤，或手動執行 GitHub 的 `Release (Python)` 工作流程並設定 `publish=false`，即可建置全部四個 wheel 套件，在 Python 3.10 和 3.14 上安裝 Linux 發行集合，檢查精確檔名和元資料，執行 PyPI 預設單檔案大小限制，並保留一份帶 SHA-256 雜湊的彙總產物。兩條路徑都沒有登錄檔憑據，Pull Request執行無法進入任何發布作業。
