...
- **完整的剛性範本**（每個生命週期使用固定章節順序，重構每份 Agent Note 以適配）：否決。大型設計 Agent Note 包含八到十五個訂製技術章節（包拓撲、協定約定、schema），它們是承載設計的內容，而非漂移；剛性順序會迫使我們現在進行破壞性改寫，並永遠與範本較勁。
...
- **獨立的 `FORMAT.md` 約定文件**：否決。由一個入口同時承載版面設定、分類和格式，比維護兩個約定文件更易發現和維護。
