...
[English](2026-07-19-cooperative-tool-cancellation.md) | 繁體中文
...
取消可能發生在策略之前、審批期間、環繞調度等待期間、工具主體啟動之後，或後置策略等待期間。單一的 `ABORTED` 結果無法讓持久化結果的消費端判斷工具主體是否可能產生過副作用。讓工具 promise 與取消競速也不是安全的後備方案，因為登錄檔報告完成後，被丟棄的同行程工作仍會繼續執行。

## 決策

`ToolExecutionInput.signal` 是必填且只讀的 `AbortSignal`，因此 `ToolExecution.signal` 和 `ToolRunContext.signal` 也都是必填且只讀。每個類型化呼叫方顯式提供自己持有的訊號；登錄檔不提供重載、預設控制器、永不中止哨兵或便捷執行路徑。
...
登錄檔信任這份類型化同行程約定。它不在執行時期校驗 `AbortSignal`，也不為缺失或畸形訊號新增敵意輸入測試。校驗仍位於解析器與設定、模型與工具 JSON、持久化與文件、worker、行程和協定邊界；違反 TypeScript 介面的無類型 JavaScript 不享有相容性約定。
...
工具主體一旦啟動，登錄檔就會等待它完成。取消透過融合訊號到達工具主體，但登錄檔不會與其 promise 競速或丟棄該 promise。協作式實作會停止自身工作或繼續轉發取消，並在所持有的工作完全靜止後完成；不協作的同行程實作可能讓登錄檔無限期保持等待。行程、worker、網路和提供方層仍負責各自的終止機制。
...
任何登錄檔測試都無法證明任意第三方同行程程式碼會觀察訊號或在有界時間內停止。各能力的測試仍需在擁有相應副作用的邊界證明取消與完全靜止。

## 考慮過的替代方案

**保留選填訊號並生成後備值。** 不予採納，因為登錄檔持有的後備訊號不代表任何呼叫方生命週期，也會保留類型系統本應阻止的缺失情況。

**在執行時期校驗 `AbortSignal`。** 不予採納，因為這是類型化同行程邊界，不是序列化邊界。執行時期檢查只會重複靜態約定，仍無法強制實作協作式使用訊號。
