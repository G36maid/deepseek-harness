# Agent Note: Web 殼產物的區塊拆分與目錄版面設定
...
- `index`（預設區塊）= react 族（react、react-dom、scheduler、use-sync-external-store）、vendored cordis、全部 workspace 程式碼及未列入的小件（anser、clsx）。
...
**目錄版面設定**（`chunkFileNames` + `assetFileNames`）：
...
- **KaTeX 整體延遲載入、boot TypeScript 文法轉懶**：會改變首幀渲染行為（公式/首個程式碼塊的回退），是獨立於產物版面設定的取捨，另行決策。
