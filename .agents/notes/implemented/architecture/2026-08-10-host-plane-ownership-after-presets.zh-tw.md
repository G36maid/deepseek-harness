...
[English](2026-08-10-host-plane-ownership-after-presets.md) | 繁體中文
...
**對未加入的 agent 否決發布。** 大聲勝過靜默，登錄檔也支援這麼做——同步的 `agent/created` 監聽器拋出會把建立整體回滾。否決的理由是：在名單之外組裝 agent 是合法的——`recompose` 寫明瞭它隨後綁定的那個裸 agent，而 ACP 橋、SDK server 與 headless bundle 今天都會建立一個。否決會把能力缺口變成一次故障。
