...
[English](2026-07-20-remove-stdio-and-echo-agents.md) | 繁體中文

## 問題

DeepSeek Harness 在 TUI 和 Headless coding agent 之外，還提供了兩個重複的產品 agent（代理）。面向行的 stdio agent 使用混合的提示符/輸出協定，同時重複實作終端機互動與非互動執行。Echo 則以無需聯網的 mock 模型加一個教學工具重複實作 Headless，把測試 fixture（測試前置資料）變成面向使用者的 agent 和預設快速上手路徑。

兩個 agent 的配套實作都不止葉節點設定。stdio 擁有 UI 外掛程式、app 包（package）、SDK 介面、REPL 葉節點、提示符協定和 Loader 測試。Echo 擁有可執行命令、mock 配接器、工具、CI 示範閘門、圖譜條目、教學引用和共享測試 fixture。保留其中任何產品路徑，都會間接保留這個重複的 agent。

標準輸入輸出仍是 ACP、JSON-RPC、MCP 和子行程的協定邊界。確定性模型配接器也仍可用於測試。這些機制不足以成為保留面向行或僅使用 mock 的產品 agent 的理由。
...
- [`@deepseek-ai/dsh-acp-demo`](../../../../packages/examples/acp-demo/README.md) 和 `@deepseek-ai/dsh-sdk-jsonrpc-server` 負責各自的分幀協定整合。

承載 `stdio` 執行介面選項的 SDK 項目模型已由 [SDK 項目工具鏈移除決策](2026-08-11-remove-sdk-project-toolchain.md)刪除。倉庫中的示範文件要求 DeepSeek API key，並優先引導到當前可執行的產品。
...
TUI 與 Headless 的 Loader 覆蓋以原始碼和建置產物兩種模式執行真實 app 包。由 PTY 驅動的子行程覆蓋僅用於 TUI 生命週期；其他入口冒煙測試使用單次管道協定。Headless 驗證任務/結果約定和工具呼叫約定。生成圖譜與倉庫搜尋會拒絕過時的包、命令、葉節點、SDK 介面、`createStdioChat` 和 `StdioRuntime` 引用。
...
- **保留 Echo 作為無金鑰快速上手路徑**：不予採納，因為首次產品體驗應使用真實模型和受支援的 coding agent，而不是帶專用工具的指令碼化配接器。
- **只為 CI 示範命令保留 Echo**：不予採納，因為由測試持有的 Headless fixture 可以覆蓋相同的 Loader 和建置產物邊界，無需保留 mock 產品葉節點。
- **移除所有 stdio 或 mock 機制**：不予採納，因為分幀協定、行程 I/O 和確定性測試配接器是獨立基礎設施，並不是被移除的 agent。

## 後果

- 互動式與非互動式產品執行分別只有一個歸屬方和一個可執行的 coding 葉節點。
- 倉庫沒有面向使用者的無金鑰 agent 示範；本機 agent 示範需要 `DEEPSEEK_API_KEY`。
