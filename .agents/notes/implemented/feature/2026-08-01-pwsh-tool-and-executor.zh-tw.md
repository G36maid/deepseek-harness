...
本決策之後的路線圖——讓 Windows 主機預設 `pwsh`（關閉 bash）與 pwsh TUI/GUI 渲染——已另行記錄為 [Windows 預設 pwsh 決策](2026-08-01-windows-pwsh-default.md)。
...
**現在就接入交付的 CLI（命令列介面）組合。** 否決：在 Windows 預設決策落地前把 `tool-pwsh` + `pwsh-local` 掛進 `base.cordis.yml` 會改變交付清單；本改動交付能力與接線點（`apps/cli` 相依性、tsconfig 工程），不切換任何預設。
...
- CLI 增加兩個 workspace 相依性與兩個 tsconfig 工程，但不掛載任一外掛程式——組合決策留給 Windows 預設提案。
