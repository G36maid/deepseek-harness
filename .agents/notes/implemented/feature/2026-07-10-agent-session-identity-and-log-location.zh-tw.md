...
一項無金鑰的完整迴圈整合測試會在第一個輪次驅動真實的 agent loop、JSONL 持久化、tool-bash 與 bash-local。子行程列印 `DSH_HOME`、`DSH_SHELL`、工作階段 id、JSONL 目標和繼承的過時哨兵值；測試校驗當前值、過時變數不存在、刷寫前文件不存在，並最終檢查持久化 header。快照測試會固定錄制請求 header 中的通用 bash 說明。該約定屬於確定性的本機執行，不涉及模型選擇，因此無需帶金鑰測試。
