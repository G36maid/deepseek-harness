...
[English](2026-08-11-deepseek-request-user-id-header.md) | 繁體中文

## 問題

當呼叫方提供 `GenerateOptions.sessionId` 時，直連 DeepSeek 請求已攜帶 `x-deepseek-harness-session-id`，讓提供方側支援與診斷可以關聯同一對話中的多個輪次。但請求缺少跨工作階段的穩定身份，而 harness 已為遙測與回饋持久化匿名使用者 id。另行生成 id 會破壞關聯；把它放進提供方無關的歸屬輔助函式，則會讓每個 HTTP 配接器都發送穩定的逐使用者標識。
...
- DeepSeek 支援可以透過一個匿名 harness-home id 跨工作階段關聯請求，並透過持久化 session id 關聯同一對話。
