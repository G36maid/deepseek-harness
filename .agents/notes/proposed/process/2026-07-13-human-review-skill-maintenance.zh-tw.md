...
[English](2026-07-13-human-review-skill-maintenance.md) | 繁體中文
...
工具原始碼、配接器二進位檔案、提供方憑據和預期的每日調度器保存在維護者機器上，不會提交到本倉庫。本文規定協定，參考實作屬於私有基礎設施。該機制只服務於由單個運維方維護的一項 skill，因此，持續透過倉庫評審審查機制改動的成本高於提交工具及其歷史的收益。如果該機制將來移交給第二位維護者，移交工作需要一篇後續 Agent Note 來修訂本決策；任何接手者都應從運維文件 [docs/cookbook/maintaining-dsh-code-review.md](../../../../docs/cookbook/maintaining-dsh-code-review.md) 入手。
...
- **讓同一個評審人既當作者又作最終裁決**：不予採納，因為獨立判定能在不受支援的概括進入 skill 之前暴露問題。
...
- **超出位元組雜湊差異的雙評審人獨立性屬於部署約定。** 兩個命令解析為逐位元組相同的可執行文件時，工具拒絕執行，但它無法驗證兩個不同包裝層是否由不同提供方或模型驅動。運維方必須設定相互獨立的主配接器與次配接器。
